...
if (isset($_POST['rsms_settings_save']) && check_admin_referer('rsms_settings','rsms_settings_nonce')) {
  ...
  update_option('rsms_msgway_send_url', esc_url_raw($_POST['send_url'] ?? 'https://api.msgway.com/send'));
  update_option('rsms_msgway_tpl_url', esc_url_raw($_POST['tpl_url'] ?? 'https://api.msgway.com/template/get'));
...
$auth=get_option('rsms_auth_token',''); $send=get_option('rsms_msgway_send_url','https://api.msgway.com/send'); $tpl=get_option('rsms_msgway_tpl_url','https://api.msgway.com/template/get'); $channel=get_option('rsms_default_channel','sms'); $rate=intval(get_option('rsms_rate_limit',60));
...
<tr><th>توکن احراز</th><td><input type="password" name="auth_token" class="rsms-input" value="<?php echo esc_attr($auth); ?>"></td></tr>
<tr><th>آدرس ارسال</th><td><input type="text" name="send_url" class="rsms-input" value="<?php echo esc_attr($send); ?>"></td></tr>
<tr><th>آدرس دریافت الگو</th><td><input type="text" name="tpl_url" class="rsms-input" value="<?php echo esc_attr($tpl); ?>"></td></tr>
